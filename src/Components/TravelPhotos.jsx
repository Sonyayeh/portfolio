...
const TravelPhotos = () => {
  ...
  return (
    <div className="bg-purple-100 pt-[1rem] w-full 
      ...
      lg:w-[85rem] lg:align-middle lg:mt-[2rem] lg:ml-[-11rem]
      lp:w-full lp:align-middle lp:mt-[2rem]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 lg:px-10 gap-4 w-full">
        {photos.map((photo) => (
          <div key={photo.id} className="relative cursor-pointer flex justify-center w-full"
      ...
      <div>
        ...
        <Link to="/travelphotofolder">
          <img src={Floppy} alt="Floppy disk, click here to my secret photo album >:)"
            className="w-8 h-8 mb-2 ml-auto sm:ml-[21rem] md:ml-[52rem] lg:ml-[83rem] lp:ml-[80rem] hover:cursor-pointer" />
        </Link>
      </div>
      
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={closeLightbox}>
          <div className="bg-white p-4 rounded-lg shadow-lg relative flex flex-col items-center max-w-[60%] max-h-[90%]"
            onClick={(e) => e.stopPropagation()}>
            <button onClick={closeLightbox} className="absolute top-2 right-2 text-white font-bold text-xl">X</button>
            <div className="relative flex flex-col items-center">
              <img src={currentImage} alt={currentAlt}
                ...
                style={{ maxWidth: "100%", maxHeight: "80vh", objectFit: "contain", border: "4px solid #fff", borderRadius: "10px" }} />
