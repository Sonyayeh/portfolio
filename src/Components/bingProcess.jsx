...
const BingProcess = () => {
  ...
  return (
    <div className="relative ml-[4rem] w-[19rem] mx-auto md:w-[46rem] md:ml-[-2rem]">
      <h2 className="text-center text-2xl font-bold mb-4 w-[17rem] text-purple-400 underline decoration-dashed sm:ml-[1rem] md:text-[3rem] md:w-[35rem] md:pb-[2rem] md:ml-[8rem] lp:text-[5rem] lp:w-[60rem] lp:mt-[6rem] lp:pb-[3rem] lp:justify-center lp:items-center lp:text-center lg:justify-center lg:items-center lg:ml-[5rem] lg:text-7xl lg:w-[60rem] lg:mb-[5rem] lg:mt-[2rem]">
        Design Process:
      </h2>

      <h1 className="text-emerald-500 sm:text-[1rem] sm:w-[23rem] sm:ml-[-2rem] md:text-[2rem] md:w-[55rem] md:ml-[3rem] lp:text-[2.5rem] lp:w-[60rem] lp:ml-[9rem] lg:text-[3rem] lg:w-[80rem] lg:ml-[2rem] lg:mt-[-2rem] lg:mb-[2rem]">
        This section displays the design process of this project. It includes everything, from <span className="text-purple-400">the takeaways of the projects </span> to <span className="text-purple-400">tasks</span>, <span className="text-purple-400"> recommendations </span>, <span className="text-purple-400">before and after comparisons </span> and <span className="text-purple-400"> conclusions </span>.
      </h1>

      <div className="overflow-x-auto flex space-x-4 py-4 scrollbar-visible sm:w-[290px] md:w-[710px] md:ml-[2rem] lp:w-[50rem] lp:ml-[13rem] lg:w-[70rem] lg:ml-[0rem]">
        {images.map((image, index) => (
          <div key={index} className="flex-shrink-0 ml-[0rem] sm:w-[280px] md:w-[700px] md:gap-x-3 md:pl-[3rem] lp:w-[30rem] lg:w-[40rem]">
            <img
              src={image.src}
              alt={image.alt}
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 lightbox-overlay hover:cursor-pointer" onClick={handleClickOutside}>
          <button className="absolute top-4 right-4 text-white text-3xl z-[9999]" onClick={closeLightbox}>X</button>

             {/* Left Arrow */}
            <div
            className="absolute top-1/2 left-[-5rem] transform -translate-y-1/2 text-5xl text-blue-500 hover:text-orange-500 hover:cursor-pointer lg:left-[-15rem] lp:left-[10rem] block"
            onClick={() => {
                handlePrev();
            ...
            {/* Right Arrow */}
            <div
            className="absolute top-1/2 right-[-5rem] transform -translate-y-1/2 text-5xl text-blue-500 hover:text-orange-500 hover:cursor-pointer lg:right-[-40rem] lp:right-[-30rem] block"
            onClick={() => {
                handleNext();
                scrollRef.current?.scrollBy({ left: 200, behavior: "smooth" });
            }}
          ...
          {/* Right Arrow */}
          <div
            className="hidden lp:block lg:block absolute top-1/2 right-[-5rem] transform -translate-y-1/2 text-5xl text-blue-500 hover:text-orange-500 hover:cursor-pointer lg:right-[-40rem] lp:right-[-30rem]"
            onClick={() => {
              handleNext();
              scrollRef.current?.scrollBy({ left: 200, behavior: "smooth" });
            }}
